...
async fn test_synthetic_pr_full_workflow_with_real_git() {
    ...
    let result = request_review(review_params).await;
    
    assert!(result.is_ok(), "request_review should succeed in test Git repo: {:?}", result.err());
    ...
    assert!(!response.files_changed.is_empty(), "Should have files_changed");
    
    // Validate we found the expected files
    assert!(response.files_changed.len() >= 2, "Should have at least 2 changed files: {:?}", response.files_changed);
    ...
    let has_auth_file = response.files_changed.iter().any(|f| f.path.contains("auth.rs"));
    let has_payment_file = response.files_changed.iter().any(|f| f.path.contains("payment.rs"));
    ...
    for thread in &response.comment_threads {
        println!("  - {:?} at {}:{}: {}", thread.comment_type, thread.file_path, thread.line_number, thread.content);
    }
    
    // Validate we found AI insight comments (be more lenient for now)
    assert!(!response.comment_threads.is_empty(), "Should have comment_threads");
    
    // Check for specific comment types if we have enough comments
    if response.comment_threads.len() >= 4 {
        let has_explanation = response.comment_threads.iter().any(|thread| 
            matches!(thread.comment_type, CommentType::Explanation)
        );
        let has_question = response.comment_threads.iter().any(|thread| 
            matches!(thread.comment_type, CommentType::Question)
        );
        let has_todo = response.comment_threads.iter().any(|thread| 
            matches!(thread.comment_type, CommentType::Todo)
        );
        let has_fixme = response.comment_threads.iter().any(|thread| 
            matches!(thread.comment_type, CommentType::Fixme)
        );
    ...
    assert!(status_result.is_ok(), "get_review_status should succeed: {:?}", status_result.err());
    
    let status = status_result.unwrap();
    assert!(status.review_id.is_some(), "Should have active review_id");
    assert_eq!(status.title, Some("Add AI insight comments and payment module".to_string()));
    assert_ne!(status.status, "no_active_review", "Should have active review");
...
async fn test_git_service_commit_range_parsing() {
    ...
    assert!(head_result.is_ok(), "Should parse HEAD: {:?}", head_result.err());
    assert!(head_tilde_result.is_ok(), "Should parse HEAD~1: {:?}", head_tilde_result.err());
    assert!(range_result.is_ok(), "Should parse HEAD~1..HEAD: {:?}", range_result.err());
...
async fn test_comment_parser_with_real_files() {
    ...
    assert!(auth_threads.is_ok(), "Should parse auth.rs: {:?}", auth_threads.err());
    let auth_threads = auth_threads.unwrap();
    assert!(!auth_threads.is_empty(), "Should find comments in auth.rs");
    
    assert!(payment_threads.is_ok(), "Should parse payment.rs: {:?}", payment_threads.err());
    let payment_threads = payment_threads.unwrap();
    assert!(!payment_threads.is_empty(), "Should find comments in payment.rs");
    
    assert!(all_threads.is_ok(), "Should parse all files: {:?}", all_threads.err());
    let all_threads = all_threads.unwrap();
    assert_eq!(all_threads.len(), auth_threads.len() + payment_threads.len(), "Should combine all comment threads");
...
async fn test_custom_repository_spec() {
    ...
    let temp_dir = TestRepo::new()
        .overwrite_and_add("lib/utils.ts", r#"
...
"#)
        ...
        .overwrite_and_add("scripts/process_data.py", r#"
...
"#)
    ...
    assert!(result.is_ok(), "Should create review from custom spec: {:?}", result.err());
    
    let response = result.unwrap();
    
    // Should find both TypeScript and Python files
    let has_ts_file = response.files_changed.iter().any(|f| f.path.contains(".ts"));
    let has_py_file = response.files_changed.iter().any(|f| f.path.contains(".py"));
    assert!(has_ts_file, "Should find TypeScript file");
    assert!(has_py_file, "Should find Python file");
    
    // Debug: Print what we found
    println!("Custom repo found {} comment threads:", response.comment_threads.len());
    for thread in &response.comment_threads {
        println!("  - {:?} at {}:{}: {}", thread.comment_type, thread.file_path, thread.line_number, thread.content);
    }
    ...
    if response.comment_threads.is_empty() {
        ...
        assert!(!response.comment_threads.is_empty(), "Should find AI comments in multi-language repo");
    }
...
async fn test_unstaged_changes_workflow() {
    ...
    let temp_dir = TestRepo::new()
        .overwrite_and_add("src/main.rs", r#"
fn main() {
    println!("Hello, world!");
}
"#)
        .commit("Initial commit")
        .overwrite_and_add("src/lib.rs", r#"
// New library module
...
"#)
        .overwrite("src/experimental.rs", r#"
...
"#)
    ...
    assert!(result.is_ok(), "Should create review with unstaged changes: {:?}", result.err());
    
    let response = result.unwrap();
    
    // Debug: Print what we found
    println!("Unstaged changes test found {} files:", response.files_changed.len());
    for file in &response.files_changed {
        println!("  - {} ({:?})", file.path, file.status);
    }
    ...
    let has_main_changes = response.files_changed.iter().any(|f| f.path.contains("main.rs"));
    let has_experimental = response.files_changed.iter().any(|f| f.path.contains("experimental.rs"));
    
    // At minimum should find some changes (either staged or unstaged)
    assert!(!response.files_changed.is_empty(), "Should find file changes including unstaged");
...
async fn test_server_info_includes_synthetic_pr_tools() {
    ...
    assert!(instructions.contains("request_review"), "Should mention request_review tool");
    assert!(instructions.contains("update_review"), "Should mention update_review tool");
    assert!(instructions.contains("get_review_status"), "Should mention get_review_status tool");
    assert!(instructions.contains("synthetic pull requests"), "Should mention synthetic PR functionality");
