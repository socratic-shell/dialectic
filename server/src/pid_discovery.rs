...
pub async fn find_vscode_pid_from_mcp(start_pid: u32) -> Result<Option<(u32, u32)>> {
    ...
    for _i in 0..10 {
        ...
        match output {
            Ok(output) if output.status.success() => {
                ...
                if lines.len() < 2 {
                    info!("No process info found for PID {}", current_pid);
                    break;
                }
                ...
                if parts.len() < 4 {
                    info!("Malformed process line: {}", process_line);
                    break;
                }
                ...
                info!("  PID {} -> PPID {}: {}", pid, ppid, command);
    ...
    info!("Reached end of process tree without finding VSCode");
    Ok(None)
}
...
fn find_terminal_shell_in_chain(process_chain: &[(u32, u32, String)], _vscode_pid: u32) -> Option<u32> {
