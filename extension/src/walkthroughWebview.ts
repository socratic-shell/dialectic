...
export class WalkthroughWebviewProvider implements vscode.WebviewViewProvider {
     ...
     */
    private async sendReferenceToActiveShell(key: string, value: any): Promise<void> {
        ...
        if (aiEnabledTerminals.length === 1) {
            ...
            if (shellPID) {
                this.daemonClient.sendStoreReferenceToShell(shellPID, key, value);
    ...
    private async sendCommentToShell(text: string, thread: vscode.CommentThread): Promise<void> {
        try {
            ...
            await this.sendReferenceToActiveShell(referenceId, {
                file: filePath,
                line: lineNumber,
                selection: undefined,
                user_comment: text
